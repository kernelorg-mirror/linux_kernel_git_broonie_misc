...
static bool vcpu_fp_user_format_needed(struct kvm_vcpu *vcpu)
{
	...
	/*
	 * If we have bboth SVE and SME and the two VLs are the same
...
{
	void *new_buf;
	int in_offset, out_offset, copy_size, i;
	...
	if (vq_in < vq_out) {
		in_offset = 0;
		out_offset = (vq_out - vq_in) * __SVE_VQ_BYTES;
		copy_size = vq_in * __SVE_VQ_BYTES;
	} else {
		in_offset = (vq_in - vq_out) * __SVE_VQ_BYTES;
		out_offset = 0;
		copy_size = vq_out * __SVE_VQ_BYTES;
	}
	for (i = 0; i < SVE_NUM_ZREGS; i++)
		memcpy(buf_zreg(new_buf, vq_out, i) + out_offset,
		       buf_zreg(vcpu->arch.sve_state, vq_out, i) + in_offset,
		       copy_size);
	...
	for (i = 0; i <= SVE_NUM_PREGS; i++)
		memcpy(buf_preg(new_buf, vq_out, i) + out_offset,
		       buf_preg(vcpu->arch.sve_state, vq_out, i) + in_offset,
		       copy_size);
...
void kvm_arch_vcpu_put_fp(struct kvm_vcpu *vcpu)
{
	...
	if (vcpu->arch.fp_state == FP_STATE_GUEST_OWNED) {
		...
		if (vcpu_has_sme(vcpu)) {
			...
			/* Restore the setup when bound to the CPU */
			if (!has_vhe()) {
				smcr_cur = read_sysreg_s(SYS_SMCR_EL1);
